...
def generate_page(from_path, template_path, dest_path, basepath):
    ...
    final_final_html = final_html.replace('href="/', f'href="{basepath}').replace('src="/', f'src="{basepath}')
    os.makedirs(os.path.dirname(dest_path), exist_ok=True)
    with open(dest_path, "w") as f:
        f.write(final_final_html)
...
def generate_pages_recursive(dir_path_content, template_path, dest_dir_path, basepath):
    for content_file in os.listdir(dir_path_content):
        ...
        if os.path.isdir(os.path.join(dir_path_content, content_file)):
            generate_pages_recursive(os.path.join(dir_path_content, content_file), template_path, os.path.join(dest_dir_path, content_file), basepath)
